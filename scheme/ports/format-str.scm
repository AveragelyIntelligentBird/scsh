...
;; * an integer, represeting an open file descriptor;
...
;; ----------------------------------------------------------------------------
;; Code below largely comes from scheme48's format.scm  

; The entry point.  Gets the port and writes the output.
; Get the appropriate writer for the port specification.

(define (format fd/port string . args)
  (cond 
    ((output-port? fd/port)
      (real-format fd/port string args))
    ((integer? fd/port) 
      (write-string 
    ...
    (else
      (assertion-violation 'format "invalid port argument" fd/port))))
